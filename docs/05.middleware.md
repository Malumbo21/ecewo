...
`use()` also accepts an optional path prefix as its first argument. The middleware will only run for requests whose path starts with that prefix — similar to Express.js's `app.use('/prefix', fn)`.

```c
void api_only_middleware(Req *req, Res *res, Next next) {
  const char *auth = get_header(req, "Authorization");
...
int main(void) {
  ...
  use("/api", api_only_middleware);

  get("/api/users", users_handler);   // middleware runs
  get("/api/posts", posts_handler);   // middleware runs
  get("/public", public_handler);     // middleware does NOT run
...
- `use("/api", fn)` matches `/api`, `/api/users`, `/api/v2/items`, ...
- `use("/api", fn)` does **not** match `/apiv2` or `/api-old`
- `use("/", fn)` matches every path (equivalent to `use(fn)`)
- `use(fn)` (no path) is global — runs for all requests
